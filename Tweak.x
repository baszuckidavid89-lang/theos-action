...
- (void)setupCircleButton {
    // Create a perfect circle button
    self.circleToggleButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.circleToggleButton.frame = CGRectMake(30, 70, 70, 70); // Placed for 11 Pro Max notch safety
    self.circleToggleButton.backgroundColor = [UIColor purpleColor];
    
    // Circle styling
    self.circleToggleButton.layer.cornerRadius = 35; // Half of width/height
...
- (void)setupMainSection {
    ...
    self.container.hidden = YES; // Start hidden
    ...
    // Input Fields
    self.xIn = [self addField:@"X Coordinate" at:60];
    self.yIn = [self addField:@"Y Coordinate" at:100];
    self.zIn = [self addField:@"Z Coordinate" at:140];
    ...
    self.qtySlider.maximumValue = 100; // Updated to 100
    ...
    genBtn.titleLabel.font = [UIFont boldSystemFontOfSize:20];
...
- (void)toggleMenu {
    ...
    // The circle button stays visible to re-open the menu
...
- (void)onSpawn {
    ...
    for (int i = 0; i < qty; i++) {
        ...
        jitter.x += (i * 0.15f); // Tiny physics offset
...
- (void)onUnityUpdate {
    ...
    dispatch_once(&once, ^{
        ...
        // Ensure menu is on the absolute top layer
